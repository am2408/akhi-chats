    ...
    <div style={{ position: "relative", flexShrink: 0 }}>
      ...
      {status && (
        <div style={{
          ...
          width: size > 32 ? 14 : 10, height: size > 32 ? 14 : 10,
          borderRadius: "50%",
...
function IconButton({ icon, title, onClick, danger }: {
  ...
  return (
    <button onClick={onClick} title={title} style={{
      ...
      background: "var(--dc-bg-tertiary)", border: "none", cursor: "pointer",
      display: "flex", alignItems: "center", justifyContent: "center",
...
export default function FriendsPage() {
  ...
  // SSE: live updates for friends and status
  useEvents({
    userId: user?.id,
    onFriendsUpdate: () => {
      fetchFriends();
    ...
    onStatusUpdate: (data) => {
      ...
      setFriends((prev) =>
        prev.map((f) => (statuses[f.id] !== undefined ? { ...f, status: statuses[f.id] } : f))
      );
    },
    onNotification: (data) => {
  ...
  const incomingPending = pendingFriends.filter((f) => f.direction === "incoming");
  const outgoingPending = pendingFriends.filter((f) => f.direction === "outgoing");
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
        ...
        <div style={{ display: "flex", gap: 2 }}>
          {tabs.map((t) => (
            <button key={t.key} onClick={() => setTab(t.key)} style={{
              ...
              color: tab === t.key ? (t.green ? "#23a55a" : "var(--dc-text-primary)") : (t.green ? "#23a55a" : "var(--dc-text-secondary)"),
              display: "flex", alignItems: "center", gap: 4, lineHeight: "22px",
            }}>
              {t.label}
              {t.count !== undefined && t.count > 0 && (
                <span style={{
                  background: "#f23f43", color: "#fff", borderRadius: 8,
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "16px 20px 16px 30px" }}>
        ...
        {tab === "add" && (
          <div>
            <h2 style={{ fontSize: 15, fontWeight: 700, textTransform: "uppercase", color: "var(--dc-text-primary)", marginBottom: 8 }}>Add Friend</h2>
            <p style={{ fontSize: 14, color: "var(--dc-text-secondary)", marginBottom: 16 }}>You can add friends by their username.</p>
            <div style={{
              display: "flex", background: "var(--dc-input-bg)", borderRadius: 8,
              border: "1px solid var(--dc-input-bg)", padding: "0 4px 0 16px", alignItems: "center", marginBottom: 24,
            }}>
              <input value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)}
                placeholder="You can add friends by their username..."
                style={{ flex: 1, padding: "12px 0", background: "transparent", border: "none", outline: "none", fontSize: 15, color: "var(--dc-text-primary)" }} />
              <button disabled={searchQuery.length < 2} style={{
                padding: "6px 16px", borderRadius: 4, border: "none", background: "#5865f2",
                opacity: searchQuery.length >= 2 ? 1 : 0.5, color: "#fff", fontSize: 13, fontWeight: 600,
                cursor: searchQuery.length >= 2 ? "pointer" : "not-allowed",
              }}>Search</button>
            </div>
            <div style={{ borderTop: "1px solid var(--dc-divider)", paddingTop: 16 }}>
              {searchLoading && <div style={{ textAlign: "center", color: "var(--dc-text-muted)", padding: 20 }}>Searching...</div>}
              {!searchLoading && searchResults.length === 0 && searchQuery.length >= 2 && (
                <div style={{ textAlign: "center", padding: 40, color: "var(--dc-text-muted)" }}>
              ...
              {searchResults.map((u) => (
                <div key={u.id} style={{
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                    <AvatarCircle avatar={u.avatar} username={u.username} size={40} status={u.status} />
                    <span style={{ fontWeight: 600, fontSize: 15 }}>{u.username}</span>
                  </div>
                  <button onClick={() => addFriend(u.id)} style={{
                    padding: "6px 16px", borderRadius: 4, border: "1px solid #23a55a",
                    background: "transparent", color: "#23a55a", cursor: "pointer", fontWeight: 600, fontSize: 13,
                  }}>Send Friend Request</button>
                </div>
              ))}
              {searchQuery.length < 2 && !searchLoading && (
                <div style={{ textAlign: "center", padding: 40, color: "var(--dc-text-muted)" }}>
                  <div style={{ marginBottom: 16, opacity: 0.5, display: "flex", justifyContent: "center" }}>{Icons.friends}</div>
                  <p style={{ fontSize: 14 }}>Type a username above to search for friends</p>
                </div>
              )}
            </div>
          </div>
        ...
        {(tab === "all" || tab === "online") && (
          <div>
            ...
            }}>{tab === "online" ? "Online" : "All Friends"} — {displayFriends.length}</h3>
            {displayFriends.length === 0 && (
              <div style={{ textAlign: "center", padding: 60, color: "var(--dc-text-muted)" }}>
                <div style={{ marginBottom: 16, opacity: 0.3, display: "flex", justifyContent: "center" }}>{Icons.friends}</div>
                <p style={{ fontSize: 14 }}>{tab === "online" ? "No one is online right now." : "You don't have any friends yet."}</p>
              </div>
            )}
            {displayFriends.map((friend) => (
              <div key={friend.friendshipId} style={{
                display: "flex", alignItems: "center", justifyContent: "space-between",
                padding: "10px 12px", borderRadius: 8, borderTop: "1px solid var(--dc-divider)", cursor: "pointer",
              }}>
                <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                  <AvatarCircle avatar={friend.avatar} username={friend.username} size={40} status={friend.status} />
                  <div>
                ...
                <div style={{ display: "flex", gap: 8 }}>
                  <IconButton icon={Icons.message} title="Message" onClick={() => router.push(`/dm/${friend.id}`)} />
                  <IconButton icon={Icons.close} title="Remove Friend" onClick={() => removeFriend(friend.friendshipId)} danger />
                </div>
              </div>
            ))}
          </div>
        ...
        {tab === "pending" && (
          <div>
            {incomingPending.length > 0 && (
              <div>
                <h3 style={{ fontSize: 11, fontWeight: 700, textTransform: "uppercase", color: "var(--dc-text-muted)", marginBottom: 8, letterSpacing: "0.02em", paddingLeft: 4 }}>
                ...
                {incomingPending.map((friend) => (
                  <div key={friend.friendshipId} style={{
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                      <AvatarCircle avatar={friend.avatar} username={friend.username} size={40} />
                      <div>
                        <div style={{ fontWeight: 600, fontSize: 15 }}>{friend.username}</div>
                    ...
                    <div style={{ display: "flex", gap: 8 }}>
                      <IconButton icon={Icons.check} title="Accept" onClick={() => acceptFriend(friend.friendshipId)} />
                      <IconButton icon={Icons.close} title="Decline" onClick={() => removeFriend(friend.friendshipId)} danger />
                    </div>
                  </div>
                ))}
              </div>
                ...
                {outgoingPending.map((friend) => (
                  <div key={friend.friendshipId} style={{
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                      ...
                      <div>
                        <div style={{ fontWeight: 600, fontSize: 15 }}>{friend.username}</div>
                        <div style={{ fontSize: 12, color: "var(--dc-text-muted)" }}>Outgoing Friend Request</div>
                      </div>
                    </div>
                    <IconButton icon={Icons.close} title="Cancel" onClick={() => removeFriend(friend.friendshipId)} danger />
                  </div>
                ))}
              </div>
            )}
            {pendingFriends.length === 0 && (
              <div style={{ textAlign: "center", padding: 60, color: "var(--dc-text-muted)" }}>
                <div style={{ marginBottom: 16, opacity: 0.3, display: "flex", justifyContent: "center" }}>{Icons.friends}</div>
                <p style={{ fontSize: 14 }}>There are no pending friend requests.</p>
              </div>
            )}
          </div>
        )}
        ...
        {tab === "blocked" && (
          <div style={{ textAlign: "center", padding: 60, color: "var(--dc-text-muted)" }}>
            <div style={{ marginBottom: 16, opacity: 0.3, display: "flex", justifyContent: "center" }}>{Icons.close}</div>
            <p style={{ fontSize: 14 }}>You can&apos;t unblock the Wumpus.</p>
          </div>
        )}
      </div>
    </div>
